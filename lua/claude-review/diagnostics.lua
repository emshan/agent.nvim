...
local function to_vim_diagnostic(diag, index)
  return {
...
function M.set(parsed_diagnostics)
  local by_file = {}

  actions.store_fixes(parsed_diagnostics)

  for idx, diag in ipairs(parsed_diagnostics) do
    local file = diag.file
    if not by_file[file] then
      by_file[file] = {}
    end
    table.insert(by_file[file], to_vim_diagnostic(diag, idx))
  end
  ...
  vim.schedule(function()
    vim.notify(
      string.format("Claude review complete: %d diagnostic(s) found", #parsed_diagnostics),
      vim.log.levels.INFO
    )
  end)
end
