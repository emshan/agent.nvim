...
function M.setup_code_action_provider()
  if code_action_provider_registered then
    return
  end
  ...
  vim.lsp.buf.code_action = function(options)
    ...
    local claude_actions = M.get_code_actions(bufnr, range)

    if #claude_actions > 0 then
      local actions_with_preview = {}
      for _, action in ipairs(claude_actions) do
        local new_action = vim.deepcopy(action)
        local original_action = action.action
        new_action.action = function()
          local diag = action.diagnostics[1]
          local file_path = vim.api.nvim_buf_get_name(bufnr)
          local key = string.format("%s:%d:%d", file_path, diag.lnum + 1, diag.col + 1)
          local fix = diagnostic_fixes[key]

          if fix then
            M.apply_fix_with_preview(bufnr, diag, fix)
          else
            original_action()
          end
        end
        table.insert(actions_with_preview, new_action)
      end

      vim.ui.select(actions_with_preview, {
      ...
      original_code_action(options)
    end
  end

  code_action_provider_registered = true
end
